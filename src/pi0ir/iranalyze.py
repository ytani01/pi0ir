...
class IrAnalyze:
    ...
    def analyze(self, raw_data=[]):
        ...
        if len(raw_data) < self.RAW_DATA_LEN_MIN:
            self._log.warning("too short:%s .. ignored", raw_data)
            return None
        ...
        self._log.debug(
            "Td=%.2f, Td_p=%.2f, Td_s=%.2f", self.Td, self.Td_p, self.Td_s
        )
        ...
        for key in self.SIG_SYM.keys():
            if self.SIG_SYM[key] in self.SIG_STR_01:
                continue
            self.sig_str = self.sig_str.replace(
                self.SIG_SYM[key], " " + self.SIG_SYM[key] + " "
            )
