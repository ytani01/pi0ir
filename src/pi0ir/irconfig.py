...
class IrConfig:
    ...
    CONF_SUFFIX = ".irconf"
    DEF_CONF_DIR = [".", str(Path.home()) + "/.irconf.d", "/etc/irconf.d"]
    ...
    def __init__(self, conf_dir=DEF_CONF_DIR, load_all=False, debug=False):
        ...
        if not isinstance(self.conf_dir, list):
    ...
    def button2syms(self, dev_data, button_name):
        ...
        """
        self._log.debug("dev_data=%s, button_name=%s", dev_data, button_name)
        ...
        if isinstance(button_data, str):
            ...
            try:
                ...
                self._log.debug(
                    "button_rep=dev_data['def_repeat']=%d", button_rep
                )
            except KeyError:
                button_rep = 1
        elif isinstance(button_data, list) and len(button_data) == 2:
        ...
        # マクロ展開
        try:
            button_str = self.expand_button_macro(
                dev_data["macro"], button_str
            )
            self._log.debug("button_str=%s", button_str)
        except KeyError:
    ...
    def get_dev(self, dev_name):
        ...
        for d_ent in self.data:
            ...
            if not isinstance(d_nlist, list):
                d_nlist = [d_nlist]
                self._log.debug("d_nlist=%s", d_nlist)
...
class App:
    ...
    def main(self, dev_name, button, conf_file):
        self._log.debug(
            "dev_name=%s, button=%s, conf_file=%s",
            dev_name,
            button,
            conf_file,
        )
...
@click.command(context_settings=CONTEXT_SETTINGS, help="IR config")
...
def main(dev_name, button, conf_file, debug):
    ...
    logger.debug(
        "dev_name=%s, button=%s, file=%s", dev_name, button, conf_file
    )
